...
/*   Updated: 2026/07/21 15:29:45 by wkerdad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	take_dongle_helper(t_coder *coder, bool lk)
{
...
static bool	take_dongles(t_coder *coder)
{
	...
	if (take_dongle_helper(coder, lk) == FAILED)
		return (FAILED);
...
int	compile(t_coder *coder)
{
	...
	compile_required = coder->data->args->number_of_compiles_required;
	if (get_int(&coder->compile_counter_mutex,
			&coder->compile_counter) >= compile_required)
