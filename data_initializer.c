...
/*   Updated: 2026/07/18 15:14:50 by wkerdad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    // initialize the hands of each coderz
    // ODD coders
    data->coders_arr[position].left_hand = &data->dongles_arr[position];
    data->coders_arr[position].right_hand = &data->dongles_arr[(position + 1) % data->args->number_of_coders];
    // EVEN coders
    if (data->coders_arr[position].coder_id % 2 == 0)
    {
        data->coders_arr[position].right_hand = &data->dongles_arr[position];
        data->coders_arr[position].left_hand = &data->dongles_arr[(position + 1) % data->args->number_of_coders];
    }
}
...
static int dongle_init(t_data *data)
{
    ...
    while (i < data->args->number_of_coders)
    {
        ...
        if (safe_mutex(INIT, &data->dongles_arr[i].dongle_mutex) == FAILED
            || safe_mutex(INIT, &data->dongles_arr[i].available_at_mutex) == FAILED
...
{
    data->coders_arr = safe_malloc(sizeof(t_coder) * data->args->number_of_coders);
    if (data->coders_arr == NULL)
        return (FAILED);
    data->dongles_arr = safe_malloc(sizeof(t_dongle) * data->args->number_of_coders);
