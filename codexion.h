...
/*   Updated: 2026/07/18 23:00:01 by wkerdad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/********************************************************************************************************/
typedef unsigned long long ull_t; // defining an abrv for the type
typedef struct s_data t_data; // declaring the typdef of data to use it recursively
typedef struct s_coder t_coder;  // declaring the typdef of coder to use it recursively
typedef struct s_dongle t_dongle;  // declaring the typdef of dongle to use it recursively
typedef struct s_args t_args;  // declaring the typdef of args to use it recursively
...
{
    t_coder *coders_arr; // coders holder
    t_dongle *dongles_arr; // dongles holder
    t_args *args; // args holder
    bool simulation_done; // a boolean to check if the sum ended no matter why
    ull_t simulation_start; // a timer for the start of the simulation
    bool all_th_rd;
